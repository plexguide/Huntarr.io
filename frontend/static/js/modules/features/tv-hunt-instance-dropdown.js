/**
 * TV Hunt instance dropdown - server-stored current instance.
 * Attach to a <select>; on change POSTs current instance then calls onChanged so the page can reload data.
 * Uses ./api/tv-hunt/instances and ./api/tv-hunt/current-instance (never localStorage).
 *
...
(function() {
    ...
    function populateSelect(select) {
        ...
        Promise.all([
            ...
            if (list.length === 0) {
                ...
                emptyOpt.value = '';
                emptyOpt.textContent = 'No TV Hunt instances';
            ...
            select.innerHTML = '<option value="">No TV Hunt instances</option>';
        });
    }
    ...
    window.TVHuntInstanceDropdown = {
        ...
        attach: function(selectId, onChanged) {
            ...
            select.addEventListener('change', function() {
                ...
                fetch(api('./api/tv-hunt/current-instance'), {
                    method: 'POST',
                    ...
                    .then(function(data) {
                        if (data.error) {
                            console.warn('[TVHuntInstanceDropdown] Set current failed:', data.error);
                            return;
                        }
                    ...
                    .catch(function(err) {
                        console.warn('[TVHuntInstanceDropdown] Set current error:', err);
                    });
            });
        },
        ...
        getCurrentInstanceId: function() {
            return fetch(api('./api/tv-hunt/current-instance') + '?t=' + Date.now(), { cache: 'no-store' })
                .then(function(r) { return r.json(); })
    ...
    // Auto-refresh all TH dropdowns when any instance changes anywhere in the app
