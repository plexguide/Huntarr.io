"""TV Hunt custom format routes (JSON; Pre-Format + Import)."""
...
from ... import trash_custom_formats_sonarr


def _custom_format_name_from_json(obj):
    """Extract display name from TV Hunt custom format JSON (top-level 'name' field)."""
...
def _recommended_score_from_json(custom_format_json):
    """Extract recommended score from TV Hunt/TRaSH custom format JSON (trash_scores.default)."""
...
def _get_custom_formats_config(instance_id):
    """Get TV Hunt custom formats list from database. Default: empty list."""
    from src.primary.utils.database import get_database
    db = get_database()
    config = db.get_app_config_for_instance('tv_hunt_custom_formats', instance_id)
...
def _save_custom_formats_config(formats_list, instance_id):
    """Save TV Hunt custom formats list to database."""
    from src.primary.utils.database import get_database
    db = get_database()
    db.save_app_config_for_instance('tv_hunt_custom_formats', instance_id, {'custom_formats': formats_list})


@tv_hunt_bp.route('/api/tv-hunt/custom-formats', methods=['GET'])
def api_tv_hunt_custom_formats_list():
    """List TV Hunt custom formats."""
    try:
        ...
        logger.exception('TV Hunt custom formats list error')
        return jsonify({'custom_formats': [], 'error': str(e)}), 200


@tv_hunt_bp.route('/api/tv-hunt/custom-formats/preformats', methods=['GET'])
def api_tv_hunt_custom_formats_preformats():
    """List TRaSH Sonarr categories (with subcategories and formats) and flat preformats."""
    try:
        categories = trash_custom_formats_sonarr.get_trash_categories()
        all_ids = trash_custom_formats_sonarr.get_all_preformat_ids()
        preformats = [{'id': pid, 'name': trash_custom_formats_sonarr.get_trash_format_name(pid) or pid} for pid in all_ids]
        return jsonify({'categories': categories, 'preformats': preformats}), 200
    except Exception as e:
        logger.exception('TV Hunt preformats list error')
        return jsonify({'categories': [], 'preformats': [], 'error': str(e)}), 200


@tv_hunt_bp.route('/api/tv-hunt/custom-formats', methods=['POST'])
def api_tv_hunt_custom_formats_add():
    ...
    try:
        ...
        if source == 'import':
            ...
            custom_format_json = trash_custom_formats_sonarr.get_trash_format_json(preformat_id)
            name = trash_custom_formats_sonarr.get_trash_format_name(preformat_id)
        ...
        logger.exception('TV Hunt custom formats add error')
        return jsonify({'success': False, 'error': str(e)}), 500


@tv_hunt_bp.route('/api/tv-hunt/custom-formats/scores', methods=['PUT'])
def api_tv_hunt_custom_formats_scores_batch():
    ...
    try:
        ...
        logger.exception('TV Hunt custom formats scores batch error')
        return jsonify({'success': False, 'error': str(e)}), 500


@tv_hunt_bp.route('/api/tv-hunt/custom-formats/<int:index>', methods=['PATCH'])
def api_tv_hunt_custom_formats_patch(index):
    ...
    try:
        ...
        logger.exception('TV Hunt custom formats patch error')
        return jsonify({'success': False, 'error': str(e)}), 500


@tv_hunt_bp.route('/api/tv-hunt/custom-formats/<int:index>', methods=['DELETE'])
def api_tv_hunt_custom_formats_delete(index):
    ...
    try:
        ...
        logger.exception('TV Hunt custom formats delete error')
        return jsonify({'success': False, 'error': str(e)}), 500


@tv_hunt_bp.route('/api/tv-hunt/custom-formats/preformats/<path:preformat_id>', methods=['GET'])
def api_tv_hunt_custom_formats_preformat_json(preformat_id):
    """Get full JSON for a TRaSH Sonarr pre-made format by id."""
    try:
        custom_format_json = trash_custom_formats_sonarr.get_trash_format_json(preformat_id)
        name = trash_custom_formats_sonarr.get_trash_format_name(preformat_id)
        if not custom_format_json:
        ...
        logger.exception('TV Hunt preformat get error')
        return jsonify({'success': False, 'error': str(e)}), 500
