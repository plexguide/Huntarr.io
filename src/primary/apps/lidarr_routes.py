...
@lidarr_bp.route('/quality-profiles', methods=['POST'])
def get_quality_profiles():
    """Get quality profiles from a Lidarr API instance"""
    data = request.json
    api_url = data.get('api_url')
    api_key = data.get('api_key')
    ...
    lidarr_logger.info(f"Fetching quality profiles from Lidarr API at {api_url}")
    ...
    try:
        ...
        from src.primary.apps.lidarr.api import get_quality_profiles
        ...
        lidarr_logger.info(f"Successfully fetched {len(profiles)} quality profiles from Lidarr")
