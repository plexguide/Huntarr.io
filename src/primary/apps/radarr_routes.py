...
@radarr_bp.route('/quality-profiles', methods=['POST'])
def get_quality_profiles():
    """Get quality profiles from a Radarr API instance"""
    data = request.json
    api_url = data.get('api_url')
    api_key = data.get('api_key')
    ...
    radarr_logger.info(f"Fetching quality profiles from Radarr API at {api_url}")
    ...
    try:
        ...
        from src.primary.apps.radarr.api import get_quality_profiles
        ...
        radarr_logger.info(f"Successfully fetched {len(profiles)} quality profiles from Radarr")
