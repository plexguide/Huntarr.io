...
@sonarr_bp.route('/quality-profiles', methods=['POST'])
def get_quality_profiles():
    """Get quality profiles from a Sonarr API instance"""
    data = request.json
    api_url = data.get('api_url')
    api_key = data.get('api_key')
    ...
    sonarr_logger.info(f"Fetching quality profiles from Sonarr API at {api_url}")
    ...
    try:
        ...
        from src.primary.apps.sonarr.api import get_quality_profiles
        ...
        sonarr_logger.info(f"Successfully fetched {len(profiles)} quality profiles from Sonarr")
